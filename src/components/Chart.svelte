<script>
    ...
    function update_data(stock, dark_theme, ticker) {
        ...
        options.subtitle.text = `Current Price: $${
            stock.current_price
        }, Change: ${(100 * (stock.price_change - 1)).toFixed(2)}%`
        options.subtitle.style.color =
            stock.price_change - 1 > 0 ? '#6aa84f' : '#cc0000'
        options = options
    }
    $: {
        update_data($stocks[ticker], $dark_theme, ticker)
    }
</script>
